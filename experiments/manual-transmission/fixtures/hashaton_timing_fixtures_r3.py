...
class State:
    ...
    def resolve_tidebinder_etb_on_hashaton_trigger(self, *, tidebinder_present: bool):
        assert self.stack and isinstance(self.stack[-1], Trigger)
        trig = self.stack.pop()
        self.events.append(f"countered:hashaton_trigger:{trig.payload}:Tidebinder")
        # The ETB counters the ability even if Tidebinder has left. The rider
        # blanks the source only if Tidebinder is still on the battlefield as
        # the ETB resolves, and only for as long as Tidebinder remains.
        if tidebinder_present:
            self.hashaton_has_abilities = False
            self.events.append("hashaton_blanked:Tidebinder")
        else:
            self.events.append("hashaton_not_blanked:Tidebinder_absent_on_ETB_resolution")
        return trig.payload

    def tidebinder_leaves_after_blanking(self):
        self.hashaton_has_abilities = True
        self.events.append("hashaton_unblanked:Tidebinder_left")
...
def fixture_led_tidebinder_does_not_erase_sibling_triggers():
    ...
    s.resolve_tidebinder_etb_on_hashaton_trigger(tidebinder_present=True)
...
def fixture_tidebinder_counters_trigger_and_blanks_source():
    ...
    assert s.resolve_tidebinder_etb_on_hashaton_trigger(tidebinder_present=True) == "Teferi, Mage of Zhalfir"
    assert not s.tokens
    assert not s.hashaton_has_abilities
...
def fixture_tidebinder_removed_before_etb_still_counters_but_does_not_blank():
    s = State()
    activate_cost_discard(s, "Putrid Imp", "Teferi, Mage of Zhalfir")
    # Tidebinder resolved and its ETB targeted the Hashaton trigger. Hashaton's
    # controller removes Tidebinder before that ETB resolves.
    assert s.resolve_tidebinder_etb_on_hashaton_trigger(tidebinder_present=False) == "Teferi, Mage of Zhalfir"
    assert s.hashaton_has_abilities
    assert not s.tokens
    # A later discard can therefore trigger Hashaton again.
    s.discard_creature("Rune-Scarred Demon", "Putrid Imp")
    s.flush_triggers()
    assert any(isinstance(x, Trigger) for x in s.stack)
    return s

def fixture_tidebinder_leaves_after_etb_hashaton_regains_abilities():
    s = State()
    activate_cost_discard(s, "Putrid Imp", "Teferi, Mage of Zhalfir")
    s.resolve_tidebinder_etb_on_hashaton_trigger(tidebinder_present=True)
    assert not s.hashaton_has_abilities
    s.tidebinder_leaves_after_blanking()
    assert s.hashaton_has_abilities
...
def fixture_tidebinder_spell_countered_creates_no_etb():
    s = State()
    activate_cost_discard(s, "Putrid Imp", "Teferi, Mage of Zhalfir")
    before = list(s.stack)
    # A countered Tidebinder spell never enters, so it produces no ETB.
    s.events.append("countered:Tishana's Tidebinder spell")
    assert s.stack == before
    assert isinstance(s.stack[-1], Trigger)
    assert s.hashaton_has_abilities
    return s
...
def main():
    ...
    fixture_tidebinder_removed_before_etb_still_counters_but_does_not_blank()
    fixtures.append("tidebinder_removed_before_etb_still_counters_but_does_not_blank")
    fixture_tidebinder_leaves_after_etb_hashaton_regains_abilities()
    fixtures.append("tidebinder_leaves_after_etb_hashaton_regains_abilities")
    fixture_tidebinder_spell_countered_creates_no_etb()
    fixtures.append("tidebinder_spell_countered_creates_no_etb")
    ...
    print(json.dumps({
        "protocol": "MT_HASHATON_TIMING_FIXTURES_R3_2026_09_23",
        "status": "PASS",
        ...
        "supersedes": "MT_HASHATON_TIMING_FIXTURES_R2_2026_09_23"
