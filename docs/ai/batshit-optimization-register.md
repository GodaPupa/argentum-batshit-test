...
- Status: **not promoted; not selected for replication; telemetry-qualified**.
- Incumbent: Variant C (`-1 Village Rites`, `+1 Shambling Ghast` relative to the permanent
...
- Disposition: Greedy Freebooter again produced the intended productive-body behavior, but removing
  Unearth #4 materially reduced useful recursion, including Flamebreather, Ghast, and Epicure
  returns. The net paired result was zero and does not justify promotion or replication.
...
No Experiment E replay or replication is authorized by this disposition, and no further Greedy
Freebooter substitution experiment is authorized at this time.
