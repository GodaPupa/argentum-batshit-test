...
Manual freeze runs `35814874094` and `35816035083` are preserved as rejected pre-entropy
dispatch-guard incidents in `tier-one-mono-blue-terror-freeze-preentropy-incident.md` and
`tier-one-mono-blue-terror-freeze-preentropy-incident-v2.md`. Runs `35817231288` and
`35818142621` are preserved in
...
The validation-only official initialization boundary is recorded in
`tier-one-grixis-official-initialization-boundary.md`. It formalizes assignment, provenance, and
...
The runner-surface audit is recorded in `tier-one-grixis-runner-surface-preflight.md`. It verifies
...
The private parser contract in `tier-one-grixis-private-synthetic-parser.md` exercises vector, CSV,
...
The disabled official-byte loader in `tier-one-grixis-disabled-official-artifact-loader.md` completed
its one authorized, read-only validation of the exact frozen artifact. The digest-only evidence is
recorded in `tier-one-grixis-official-loader-validation-provenance.json`; its manual trigger and
...
The final closure in `tier-one-grixis-final-readiness.md` audits the complete frozen harness and live
