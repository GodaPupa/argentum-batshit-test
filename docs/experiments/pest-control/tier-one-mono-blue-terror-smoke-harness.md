...
- Outcome exposure: `0`
- Provenance adapter: present, no official initialization method
- Construction-only initializer: fixed excluded fixture
- Official game initializer: absent
...
adapter's lack of an official initialization method, and the absent execution method. No test in this
gate can initialize an official game.

The provenance-only adapter and byte-level artifact contract are implemented in the following gate.
They use synthetic in-memory fixtures and expose no official game initializer. Execution remains
disabled and the vector remains absent. No smoke or official gameplay is authorized by this
document.
