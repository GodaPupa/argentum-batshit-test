# Pest Control Tier-1 coverage — runner-surface preflight

## Audit

This gate audits every repository workflow, tool source, server entry surface, and the compiled
public methods of the Grixis initialization boundary. It fails if it finds the Grixis smoke block,
execution contract, initialization boundary, or execution label on a workflow/command surface.
...
Public `run`, `execute`, `main`, official initialization, seed-generation, seed-freeze, or artifact
writer methods fail closed. Synthetic negative fixtures verify each class of exposure is detected.

## Current state

- Workflows capable of Grixis smoke execution: `0`
- Commands/server entry points capable of Grixis smoke execution: `0`
