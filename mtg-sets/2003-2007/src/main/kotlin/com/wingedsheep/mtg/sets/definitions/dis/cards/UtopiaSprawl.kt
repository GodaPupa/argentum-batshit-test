...
/** Utopia Sprawl — Dissension #99. */
val UtopiaSprawl = card("Utopia Sprawl") {
    manaCost = "{G}"
    colorIdentity = "G"
    typeLine = "Enchantment — Aura"
    oracleText = "Enchant Forest\nAs this Aura enters, choose a color.\nWhenever enchanted Forest " +
        "is tapped for mana, its controller adds an additional one mana of the chosen color."

    auraTarget = TargetPermanent(
        filter = TargetFilter(GameObjectFilter.Land.withSubtype(Subtype.FOREST))
    )
