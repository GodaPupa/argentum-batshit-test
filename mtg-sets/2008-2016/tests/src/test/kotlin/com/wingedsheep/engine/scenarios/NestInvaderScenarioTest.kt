...
class NestInvaderScenarioTest : ScenarioTestBase() {
    ...
    init {
        test("entry creates the exact colorless Spawn count for its controller") {
            val game = scenario().withRngSeed(0x4e455354L).withPlayers("Opponent", "Caster")
                .withCardInHand(2, "Nest Invader")
                .withLandsOnBattlefield(2, "Forest", 2)
                .withActivePlayer(2).inPhase(Phase.PRECOMBAT_MAIN, Step.PRECOMBAT_MAIN).build()
            game.castSpell(2, "Nest Invader").error shouldBe null
            game.findPermanents("Eldrazi Spawn") shouldHaveSize 0
            game.settle()
            game.assertSpawns(2)
            val body = game.findPermanent("Nest Invader")!!
            game.state.projectedState.getPower(body) shouldBe 2
            game.state.projectedState.getToughness(body) shouldBe 2
            game.state.projectedState.getColors(body) shouldBe setOf("GREEN")
        }

        test("newly created Spawn sacrifice immediately for usable colorless mana without using the stack") {
            val game = scenario().withRngSeed(0x4e455354L).withPlayers("Caster", "Opponent")
                .withCardInHand(1, "Nest Invader")
                .withCardInHand(1, "Sol Ring")
                .withLandsOnBattlefield(1, "Forest", 2)
                .withActivePlayer(1).inPhase(Phase.PRECOMBAT_MAIN, Step.PRECOMBAT_MAIN).build()
            game.castSpell(1, "Nest Invader").error shouldBe null
            game.settle()
            game.assertSpawns(1)
        ...
        test("the entry trigger still creates Spawn after its source dies in response") {
            ...
            game.settle()
            game.isInGraveyard(1, "Nest Invader") shouldBe true
            game.assertSpawns(1)
        }

        test("countering the creature spell creates no Spawn because it never enters") {
            val game = scenario().withRngSeed(0x4e455354L).withPlayers("Caster", "Counterspell")
                .withCardInHand(1, "Nest Invader")
            ...
            game.settle()
            game.findPermanent("Nest Invader") shouldBe null
            game.isInGraveyard(1, "Nest Invader") shouldBe true
            game.findPermanents("Eldrazi Spawn") shouldHaveSize 0
        }
    }
