...
class PyroblastScenarioTest : ScenarioTestBase() {
    private fun fixture(): ScenarioBuilder = scenario().withPlayers()
        .withRngSeed(9250925021L)
        .withCardInHand(1, "Pyroblast")
        .withLandsOnBattlefield(1, "Mountain", 1)

    private fun TestGame.castMode(mode: Int, target: ChosenTarget) = execute(CastSpell(
        player1Id, findCardsInHand(1, "Pyroblast").single(), listOf(target),
    ...
    init {
        for ((victim, shouldDestroy) in listOf("Phantom Warrior" to true, "Grizzly Bears" to false)) {
            test("permanent mode legally targets $victim and checks its blue color on resolution") {
                ...
                game.isInGraveyard(1, "Pyroblast") shouldBe true
            }
        }

        for ((victim, lands, shouldCounter) in listOf(
            Triple("Phantom Warrior", "Island", true),
            Triple("Grizzly Bears", "Forest", false)
        )) {
            test("spell mode legally targets $victim and only counters a blue spell") {
        ...
        test("a real color changing ability in response controls the destruction result") {
            val game = fixture().withCardOnBattlefield(2, "Grizzly Bears")
        ...
        test("a target returned to hand in response is never destroyed in its new zone") {
            val game = fixture().withCardOnBattlefield(2, "Phantom Warrior")
                .withCardInHand(2, "Unsummon").withLandsOnBattlefield(2, "Island", 1).build()
            val victim = game.findPermanent("Phantom Warrior")!!
            game.castMode(1, ChosenTarget.Permanent(victim)).error shouldBe null
            game.passPriority().error shouldBe null
            game.castSpell(2, "Unsummon", victim).error shouldBe null
            game.settle()
            game.isInHand(2, "Phantom Warrior") shouldBe true
            game.isInGraveyard(2, "Phantom Warrior") shouldBe false
            game.isInGraveyard(1, "Pyroblast") shouldBe true
        }

        test("the permanent mode may destroy the caster's own matching permanent") {
            val game = fixture().withCardOnBattlefield(1, "Phantom Warrior").build()
            game.castMode(1, ChosenTarget.Permanent(game.findPermanent("Phantom Warrior")!!)).error shouldBe null
            game.settle()
            game.isInGraveyard(1, "Phantom Warrior") shouldBe true
        }

        test("choosing a mode cannot substitute the other mode's target category") {
            val game = fixture().withCardInHand(2, "Grizzly Bears")
                .withLandsOnBattlefield(2, "Forest", 2).withCardOnBattlefield(2, "Phantom Warrior")
                .withActivePlayer(2).build()
            game.castSpell(2, "Grizzly Bears").error shouldBe null
            val spell = game.state.stack.single()
            game.passPriority().error shouldBe null
            val before = game.state
            game.castMode(0, ChosenTarget.Permanent(game.findPermanent("Phantom Warrior")!!)).error shouldNotBe null
            game.state shouldBe before
            game.castMode(1, ChosenTarget.Spell(spell)).error shouldNotBe null
            game.state shouldBe before
            game.isInHand(1, "Pyroblast") shouldBe true
        }
    }
}
