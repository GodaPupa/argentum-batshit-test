...
/** Deterministic coverage for Peregrine Drake's optional five-land untap trigger. */
class PeregrineDrakeScenarioTest : FunSpec({
    ...
    test("entering untaps the five chosen lands") {
        val driver = driver()
        val player = driver.activePlayer!!
        val lands = List(5) { driver.putPermanentOnBattlefield(player, "Island") }
        lands.forEach(driver::tapPermanent)

        val drake = driver.putCardInHand(player, "Peregrine Drake")
        driver.giveColorlessMana(player, 4)
        driver.giveMana(player, Color.BLUE, 1)
        driver.castSpell(player, drake).isSuccess shouldBe true
        driver.bothPass()
