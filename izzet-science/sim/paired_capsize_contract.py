#!/usr/bin/env python3
"""Paired Capsize-policy estimands and fail-closed summary contract."""
...
METRICS=("capsize_present","capsize_buyback","combo_pair","combo_lethal",
         "lethal_by_now","commander_battlefield")
...
def aggregate_paired_games(pairs,source_sha,master_seed,samples,derive_seed,through=10):
    """Aggregate strict matched pairs without discarding discordant-pair direction."""
    ...
    summary={
        ...
        "derivation":DERIVATION,"turns":turns,
    }
    validate_summary(summary,source_sha,master_seed,samples)
    return summary


def validate_summary(summary,expected_source,expected_seed,expected_samples):
    """Reject incomplete, malformed, noncanonical, or internally inconsistent output."""
    ...
    for expected_turn,row in enumerate(summary["turns"],1):
        ...
        for arm,index in (("control",0),("policy",1)):
            if arm_counts["capsize_buyback"][index]>arm_counts["capsize_present"][index]:
                raise ValueError(f"Capsize readiness subset violation arm={arm}")
