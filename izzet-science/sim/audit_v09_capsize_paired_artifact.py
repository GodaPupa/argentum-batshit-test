#!/usr/bin/env python3
"""Fail-closed audit of a downloaded paired Capsize pilot artifact."""
...
def audit_files(files,expected_source,expected_workflow,expected_runner_sha256,
    ...
    if set(files)!=FILES or any(not isinstance(value,bytes) or not value for value in files.values()):
    ...
    summary=loads_strict(files["paired-summary.json"].decode("utf-8"))
    validate_summary(summary,expected_source,expected_seed,expected_samples)
    expected_audit=(
        f"audit=pass\n"
        f"schema={SCHEMA}\n"
        ...
        f"through=10\n").encode()
    if files["paired-audit.txt"]!=expected_audit:
    ...
    expected={
        ...
        "through":10,
        "summary_sha256":digest(files["paired-summary.json"]),
        "audit_sha256":digest(files["paired-audit.txt"]),
        "runner_sha256":digest(files["paired-runner.py"]),
    }
    ...
    if (type(manifest["run_attempt"]) is not int or
...
def main() -> None:
    ...
    files={path.name:path.read_bytes() for path in args.artifact.iterdir() if path.is_file()}
    manifest=audit_files(files,args.expected_source,args.expected_workflow,
