...
/**
 * Canonical Spy engine decisions reused from the accepted Industrial source. Each experiment
 * supplies its prospectively fixed revealed-hand ranking and separately qualifies the profile.
 * Decisions read the acting player's hand, public zones, or the pending decision's legal options.
 */
class SpyComboAdvisorModule(revealedHandPriority: List<String>) : CardAdvisorModule {
    private val fiendAdvisor = SpyMesmericFiendAdvisor(revealedHandPriority.toList())
    override fun register(registry: CardAdvisorRegistry) {
        ...
        registry.register(fiendAdvisor)
...
private object SpyQuirionAdvisor : CardAdvisor {
    ...
    override fun costPaymentPreference(
        state: GameState, entityId: EntityId, playerId: EntityId, costType: String,
    ): Double? = if (costType == "BouncePermanent") {
        if (state.getEntity(entityId)?.has<TappedComponent>() == true) 100.0 else 80.0
    } else null
...
private object SpyDefenderManaAdvisor : CardAdvisor {
    ...
    override fun evaluateCast(context: CastContext): Double? {
        ...
        val defenders = context.projected.getBattlefieldControlledBy(context.playerId).count { id ->
            context.projected.isCreature(id) && context.projected.hasKeyword(id, Keyword.DEFENDER)
        }
...
private object SpyLandcyclerAdvisor : CardAdvisor {
    ...
    override fun evaluateCast(context: CastContext): Double? {
        ...
        val battlefieldLands = context.projected.getBattlefieldControlledBy(context.playerId).count { id ->
            context.projected.hasType(id, "LAND")
        }
...
private class SpyMesmericFiendAdvisor(private val priority: List<String>) : CardAdvisor {
...
private object SpyDreadReturnAdvisor : CardAdvisor {
    ...
    override fun costPaymentPreference(
        state: GameState, entityId: EntityId, playerId: EntityId, costType: String,
    ): Double? = if (costType == "SacrificePermanent") spySacrificeRank(state.spyName(entityId)) else null

    override fun respondToDecision(context: AdvisorDecisionContext) =
        (context.decision as? SelectCardsDecision)?.let { decision ->
            if (decision.maxSelections < 3) return@let null
            val selected = decision.options.sortedByDescending { id ->
                spySacrificeRank(context.state.spyName(id))
            }.take(3)
...
private fun spySacrificeRank(name: String?): Double {
    val index = SPY_SACRIFICE_ORDER.indexOf(name)
    return if (index < 0) 0.0 else (SPY_SACRIFICE_ORDER.size - index).toDouble()
}
