...
private fun playLoggedGame(
            ...
            when (event) {
                ...
                is ZoneChangeEvent -> {
                    ...
                    if (event.entityName in setOf(
                            ...
                            "Shambling Ghast", "Sneaky Snacker", "Treasure"
    ...
    while (!state.gameOver && state.turnNumber <= maxPlayerTurns && actions < maxActions) {
        ...
        check(result.error == null) { "Illegal agent action $action: ${result.error}" }
        logEvents(result.events, result.state)

        if (action is PlayLand && cardName(action.cardId) == "Razortrap Gorge") {
            val tapped = result.state.getEntity(action.cardId)?.get<TappedComponent>() != null
            if (tapped) gorgeTappedEntries++
            log.appendLine("  TEMPO Razortrap Gorge entered ${if (tapped) "tapped" else "untapped"}")
        }
        val afterPrepared = preparedCopies(result.state, batshitId)
