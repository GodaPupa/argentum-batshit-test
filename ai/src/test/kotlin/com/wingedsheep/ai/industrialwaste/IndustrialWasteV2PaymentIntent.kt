 ...
 * the ordinary solver's unqualified paid-filter paths are never treated as executable witnesses.
...
internal class IndustrialWasteV2PaymentBinder(
    ...
    private fun autoPayable(state: GameState, player: EntityId, action: GameAction, cost: ManaCost,
        ...
        // Preserve the actual AutoPay choice, rather than prove another set of sources and then
        // submit an action whose unqualified planner can choose an unfunded/circular filter.
        return solution.sources.none { it.colorActivationManaCost.values.any { amount -> amount > 0 } }
    }
