...
class IndustrialWasteV2SacrificeCheckpointTest : FunSpec({
    ...
    test("repeatable Altar mana proves the generic flashback payment and missing black") {
        val driver = game()
    ...
    test("recovery activation proves repeated explicit funding without inventing black") {
        val driver = game()
        driver.putPermanentOnBattlefield(driver.player1, "Ashnod's Altar")
        repeat(2) { driver.putPermanentOnBattlefield(driver.player1, "Myr Retriever") }
        ...
        val metric = checkpoint(driver)
        metric.relevantActivatedAbilities.single().status shouldBe IndustrialWasteV2CheckpointCardStatus.UNAVAILABLE_COLORED_PAYMENT
        metric.coloredManaFailure shouldBe true
        metric.unresolved shouldBe false
    }
