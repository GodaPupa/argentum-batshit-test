...
class CastSpellHandler(
    ...
    override fun validate(state: GameState, action: CastSpell): String? {
        ...
        val bestowForLegality = action.useAlternativeCost &&
            action.alternativeCostType == AlternativeCostType.BESTOW &&
            cardDef?.keywordAbilities?.any { it is KeywordAbility.Bestow } == true
        // CR 702.103d: casting restrictions inspect the chosen Aura spell.
        val castCharacteristicState = if (bestowForLegality) {
            state.updateEntity(action.cardId) {
                it.with(cardComponent.copy(typeLine =
                    com.wingedsheep.engine.state.components.identity.BestowComponent.auraType(cardComponent.typeLine)))
            }
        } else state
        ...
        castPermissionUtils.reasonCannotCast(castCharacteristicState, action.playerId, action.cardId)?.let { return it }
        ...
        if (!effectiveTypeLine.isInstant) {
            ...
            val grantedFlash = hasFlash || zoneResolver.hasGrantedFlash(castCharacteristicState, action.cardId)
            ...
            if (targetRequirements.isNotEmpty()) {
                ...
                val targetState = if (bestowValidation) {
                    state.updateEntity(action.cardId) { container ->
                        ...
                        else container.with(card.copy(typeLine =
                            com.wingedsheep.engine.state.components.identity.BestowComponent.auraType(card.typeLine)))
                    }
                } else state
                val validationDef = if (bestowValidation) {
                    cardDef.copy(typeLine =
                        com.wingedsheep.engine.state.components.identity.BestowComponent.auraType(cardDef.typeLine))
        ...
        var effectiveCost = if (playForFree) {
            ...
            if (action.altAllows(AlternativeCostType.FLASHBACK) && flashbackAbility != null && zoneResolver.hasFlashbackPermission(state, action.playerId, action.cardId)) {
                ...
                costCalculator.calculateEffectiveCostWithAlternativeBase(
                    state, cardDef, MayhemGrants.effectiveMayhem(state, action.cardId, cardDef, action.playerId, cardRegistry, predicateEvaluator)!!.cost, action.playerId, chosenTargetIds)
                ...
                costCalculator.calculateEffectiveCostWithAlternativeBase(
                    state, cardDef, DisturbCasts.printedDisturb(cardDef)!!.cost, action.playerId, chosenTargetIds)
                ...
                costCalculator.calculateEffectiveCostWithAlternativeBase(
                    state, cardDef, modalBackFace.manaCost, action.playerId, chosenTargetIds)
            } else {
                ...
                if (action.altAllows(AlternativeCostType.WARP) && warpAbility != null && zoneResolver.hasWarpPermission(state, action.playerId, action.cardId)) {
                    ...
                    if (action.altAllows(AlternativeCostType.SNEAK) && sneakCost != null) {
                        ...
                        if (action.alternativeCostType == AlternativeCostType.BESTOW && bestowAbility != null) {
                            val bestowDef = cardDef.copy(typeLine =
                                com.wingedsheep.engine.state.components.identity.BestowComponent.auraType(cardDef.typeLine))
                            costCalculator.calculateEffectiveCostWithAlternativeBase(
                                state, bestowDef, bestowAbility.cost, action.playerId, chosenTargetIds
                            )
    ...
    private fun validatePayment(state: GameState, action: CastSpell, cost: ManaCost, paymentXValue: Int = action.xValue ?: 0): String? {
        ...
        val bestowAbility = cardComponent?.let { cardRegistry.getCard(it.cardDefinitionId) }
            ?.keywordAbilities?.filterIsInstance<KeywordAbility.Bestow>()?.firstOrNull()
            ?.takeIf { action.useAlternativeCost && action.alternativeCostType == AlternativeCostType.BESTOW }
        val spellCtx = if (action.castFaceDown) {
            ...
            val paymentCard = if (bestowAbility != null) cardComponent.copy(
                typeLine = com.wingedsheep.engine.state.components.identity.BestowComponent.auraType(cardComponent.typeLine)
            ) else cardComponent
            SpellPaymentContext(
                ...
                hasXInCost = bestowAbility?.cost?.hasX ?: cardComponent.manaCost.hasX,
    ...
    override fun execute(state: GameState, action: CastSpell): ExecutionResult {
        ...
        var effectiveCost = if (playForFreeInExecute) {
            ...
            if (action.altAllows(AlternativeCostType.FLASHBACK) && flashbackAbility != null && zoneResolver.hasFlashbackPermission(currentState, action.playerId, action.cardId)) {
                ...
                // Mayhem cost (CR 702.187) — cast from graveyard for its mayhem cost.
                costCalculator.calculateEffectiveCostWithAlternativeBase(
                    currentState, cardDef, MayhemGrants.effectiveMayhem(currentState, action.cardId, cardDef, action.playerId, cardRegistry, predicateEvaluator)!!.cost, action.playerId, chosenTargetIdsForPayment)
            } else if (action.altAllows(AlternativeCostType.DISTURB) &&
                DisturbCasts.printedDisturb(cardDef) != null &&
                zoneResolver.disturbCastFace(currentState, action.playerId, action.cardId) != null) {
                // Disturb cost (CR 702.146a) — mirrors validate().
                costCalculator.calculateEffectiveCostWithAlternativeBase(
                    currentState, cardDef, DisturbCasts.printedDisturb(cardDef)!!.cost, action.playerId, chosenTargetIdsForPayment)
            } else if (action.altAllows(AlternativeCostType.MODAL_BACK_FACE) && modalBackFace != null) {
                // Modal DFC back face (CR 712.11b) — mirrors validate().
                costCalculator.calculateEffectiveCostWithAlternativeBase(
                    currentState, cardDef, modalBackFace.manaCost, action.playerId, chosenTargetIdsForPayment)
            } else {
                // Check warp cost (hand only — CR 702.185a). Re-casts from exile pay the regular
                // mana cost. Printed warp wins; a battlefield grant ([GrantWarpToCardsInHand])
                // supplies the cost when the card has no printed warp.
                    ...
                    if (action.altAllows(AlternativeCostType.SNEAK) && sneakCost != null) {
                        ...
                        if (action.alternativeCostType == AlternativeCostType.BESTOW && bestowAbility != null) {
                            val bestowDef = cardDef.copy(typeLine =
                                com.wingedsheep.engine.state.components.identity.BestowComponent.auraType(cardDef.typeLine))
                            costCalculator.calculateEffectiveCostWithAlternativeBase(
                                currentState, bestowDef, bestowAbility.cost, action.playerId, chosenTargetIdsForPayment
                            )
                        } else if (action.altAllows(AlternativeCostType.IMPENDING) && impendingAbility != null) {
        ...
        val bestowPayment = cardDef?.keywordAbilities?.filterIsInstance<KeywordAbility.Bestow>()?.firstOrNull()
            ?.takeIf { action.useAlternativeCost && action.alternativeCostType == AlternativeCostType.BESTOW }
        val paymentCard = if (bestowPayment != null) cardComponent.copy(
            typeLine = com.wingedsheep.engine.state.components.identity.BestowComponent.auraType(cardComponent.typeLine)
        ) else cardComponent
        val spellContext = if (action.castFaceDown) {
            ...
            isLegendary = paymentCard.typeLine.isLegendary,
            manaValue = cardComponent.manaCost.cmc,
            hasXInCost = bestowPayment?.cost?.hasX ?: cardComponent.manaCost.hasX,
            subtypes = paymentSubtypesOf(paymentCard),
        ...
        run {
            val record = com.wingedsheep.engine.state.CastSpellRecord(
                ...
                typeLine = if (bestowPayment != null) paymentCard.typeLine
