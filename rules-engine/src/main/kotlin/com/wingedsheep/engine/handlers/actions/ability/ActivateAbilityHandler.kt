...
class ActivateAbilityHandler(
    ...
    private fun executeActivation(
        ...
        if (action.paymentStrategy !is PaymentStrategy.Explicit && manaCost != null && manaCost.hasX && xValue > 0) {
            ...
            // Restricted floating mana may fund this activation only when both its spending
            // condition and this ability's X-color restriction permit it.
            for (entry in manaPool.restrictedMana.toList()) {
                if (xRemainingToPay <= 0) break
                if (!entry.restriction.isSatisfiedBy(executeAbilityContext)) continue
                if (xManaRestriction.isNotEmpty() && entry.color !in xManaRestriction) continue
                val spent = manaPool.spendRestricted(entry.color, executeAbilityContext) ?: continue
                manaPool = spent
                xRemainingToPay--
            }
            ...
            if (xRemainingToPay > 0) {
                return ExecutionResult.error(state, "Not enough eligible mana to pay X")
            }
        }
    ...
    private fun autoTapForManaCost(
        ...
        // sharing the same contextual coverage rule as ManaSolver.canPay (ManaPool.xCoverage).
