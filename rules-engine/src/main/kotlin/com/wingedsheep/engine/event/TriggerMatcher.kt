        ...
        return when (predicate) {
            ...
            is com.wingedsheep.sdk.scripting.predicates.CardPredicate.HasColor -> {
                if (isFaceDown) {
                    ...
                    // Zone entries are live in projected state, so layer-5 color changes apply.
                    // For non-battlefield cards (where projection has no value), use the card's
                    // intrinsic colors, matching the other trigger-time predicate evaluators.
                    val colors = projected.getColors(entityId).ifEmpty {
