...
class ManaSolver(
    ...
    fun canPay(
        ...
        // A finite fixed-resource query accounts for shared sacrifice materials and repeated
        // activations before the older aggregate fallback. Auto-pay remains sacrifice-free;
        // a positive result here advertises an explicit payment, never submits that payment.
        if (!cost.hasX && xValue == 0 && phyrexianLifePipsCommitted == 0) {
            when (ManaPaymentFeasibility(cardRegistry).assess(state, playerId,
                ManaPaymentRequest(cost, context = spellContext, excludedManaEntities = excludeSources))) {
                is ManaPaymentFeasibilityResult.Payable -> return true
                is ManaPaymentFeasibilityResult.Impossible -> return false
                is ManaPaymentFeasibilityResult.Unsupported -> Unit
            }
        }
