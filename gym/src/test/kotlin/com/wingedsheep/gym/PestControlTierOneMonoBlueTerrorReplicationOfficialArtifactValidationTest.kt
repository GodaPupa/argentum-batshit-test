...
private const val TERROR_REPLICATION_VALIDATE_MODE = "VALIDATE_ONLY"

class PestControlTierOneMonoBlueTerrorReplicationOfficialArtifactValidationTest : FunSpec({
    val enabled =
        System.getenv("PEST_TERROR_REPLICATION_MODE") == TERROR_REPLICATION_VALIDATE_MODE

    test("exact frozen twelve-game Terror replication artifact validates without gameplay").config(
        enabled = enabled,
    ) {
        val dir = Path.of(
        ...
        val input =
            PestControlTierOneMonoBlueTerrorReplicationExecutionInputLoader.loadValidated(dir)
        ...
        input.vectorIdentity.orderedVectorSha256 shouldBe
            PEST_MONO_BLUE_TERROR_REPLICATION_VECTOR_SHA256
        input.vectorIdentity.assignmentCsvSha256 shouldBe
            PEST_MONO_BLUE_TERROR_REPLICATION_ASSIGNMENTS_SHA256
        input.vectorIdentity.freezeManifestSha256 shouldBe
