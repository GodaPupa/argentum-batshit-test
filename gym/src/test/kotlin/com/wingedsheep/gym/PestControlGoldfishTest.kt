...
class PestControlGoldfishTest : FunSpec({
    test("frozen Pest Control v1.0 and Sample 1 seed vector are exact") {
        ...
        sample2Take7.size shouldBe 30
        sample2Take7.distinct().size shouldBe 30
        sample2Take7.intersect(retired.toSet()) shouldBe emptySet()
        sample2Take7.intersect(fresh.toSet()) shouldBe emptySet()
        sample2Take7.intersect(performance.toSet()) shouldBe emptySet()
        sample2Take7.intersect(untouched.toSet()) shouldBe emptySet()
        sample2Take7.intersect(sample2.toSet()) shouldBe emptySet()
        sample2Take7.intersect(sample2Take2.toSet()) shouldBe emptySet()
        sample2Take7.intersect(sample2Take3.toSet()) shouldBe emptySet()
        sample2Take7.intersect(sample2Take4.toSet()) shouldBe emptySet()
    ...
    test("Pest Control v1.0 Goldfish Sample 2 Take 7 independent replication").config(
        ...
        Files.createDirectories(reportDir)
        Files.writeString(
            reportDir.resolve("goldfish-sample-2-take-7-raw.json"),
            PEST_ARTIFACT_JSON.encodeToString(block),
        )
        val markdown = renderPestMarkdown(
            block,
            freshPerformanceSample = true,
            sampleLabel = "Goldfish Sample #2 — Take 7 Independent Replication",
        )
        Files.writeString(reportDir.resolve("goldfish-sample-2-take-7-report.md"), markdown)
        println(markdown)
        games.flatMap(PestGoldfishGame::auditErrors) shouldBe emptyList()
    }

    test("Pest Control v1.0 Goldfish Sample 2 Take 8 independent replication").config(
        enabled = false, // Accepted Sample #2; vector executed once and must never be run again.
        timeout = 60.minutes,
    ) {
