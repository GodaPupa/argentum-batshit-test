...
class PestControlTierOneMonoBlueTerrorDisabledSyntheticProvenanceTest : FunSpec({
    ...
    test("opaque rows bind to exact synthetic provenance without executable fields") {
        ...
        result.provenanceSha256 shouldBe
            PEST_MONO_BLUE_TERROR_DISABLED_SYNTHETIC_PROVENANCE_SHA256
        result.rows.size shouldBe 4
        result.rows.all {
            it.protocolId == PEST_MONO_BLUE_TERROR_PREBOARD_PROTOCOL_ID
        } shouldBe true
        result.rows.all {
            it.blockId == PEST_MONO_BLUE_TERROR_SMOKE_BLOCK_ID
        } shouldBe true
        result.rows.all {
            it.qualifiedRunner == PEST_V2_QUALIFIED_RUNNER
        } shouldBe true

        val fieldNames =
            MonoBlueTerrorDisabledSyntheticProvenanceRow::class.java.declaredFields
    ...
    test("runner substitution fails closed without changing official counters") {
        val result =
            PestControlTierOneMonoBlueTerrorDisabledSyntheticProvenance.inspect(
                registry,
