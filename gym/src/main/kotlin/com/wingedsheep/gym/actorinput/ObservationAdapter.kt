 ...
 */
class ObservationAdapter(private val registry: CardRegistry) {
    private val visibility = Visibility(registry)
    private val builder = ObservationBuilder(registry)
    private val stackResolver = com.wingedsheep.engine.mechanics.stack.StackResolver(registry)
    private val basicBluePayment = ActorBasicBluePaymentProjector(registry)
    ...
    fun build(
        ...
        val stack = base.stack.map { item ->
            ...
            ActorStackItem(item, trigger?.sourceId ?: activation?.sourceId,
                ...
                projectSpellCharacteristics(state, item.entityId, stackResolver, registry))
        }
        ...
        stack.forEach {
            requireAccessible(it.source, publicHandles)
            requireAccessible(it.spell, publicHandles)
        }
        ...
        val menu = legalActions.map { action ->
            val copied = action.actorCopy().copy(basicBluePayment = basicBluePayment.project(state, action))
