...
@Serializable
data class MonoBlueTerrorSmokeAttempt(val gameNumber: Int, val seed: Long)

@Serializable
data class MonoBlueTerrorSmokeArtifactIndex(
...
object PestControlTierOneMonoBlueTerrorArtifactContract {
    fun buildIndex(
        vectorIdentity: MonoBlueTerrorSmokeVectorIdentity,
        attempts: List<MonoBlueTerrorSmokeAttempt>,
        ...
        val index = MonoBlueTerrorSmokeArtifactIndex(
    ...
    fun validate(
        ...
        if (index.blockId != PEST_MONO_BLUE_TERROR_SMOKE_BLOCK_ID) add("block mismatch")
        if (index.acceptedReadinessCommit != PEST_MONO_BLUE_TERROR_READINESS_COMMIT) {
            add("accepted readiness commit mismatch")
        }
        if (index.expectedGames != PEST_MONO_BLUE_TERROR_SMOKE_GAMES) add("expected game count mismatch")
        ...
        if (frozenSeeds.size != PEST_MONO_BLUE_TERROR_SMOKE_GAMES) add("frozen vector must contain four seeds")
        ...
        if (index.attemptedGames.size > PEST_MONO_BLUE_TERROR_SMOKE_GAMES) add("too many attempts")
        ...
        when (index.disposition) {
            "VALIDATED" -> if (
                index.attemptedGames.size != PEST_MONO_BLUE_TERROR_SMOKE_GAMES ||
                index.recordedGames.size != PEST_MONO_BLUE_TERROR_SMOKE_GAMES
