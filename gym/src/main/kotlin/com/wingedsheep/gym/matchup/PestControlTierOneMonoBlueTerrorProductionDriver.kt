...
@Serializable
data class MonoBlueTerrorOfficialActionTrace(
    val sequence: Int,
    val turn: Int,
...
@Serializable
data class MonoBlueTerrorOfficialTerminal(
...
@Serializable
data class MonoBlueTerrorOfficialRawGame(
    val provenance: MonoBlueTerrorSmokeProvenance,
    val actions: List<MonoBlueTerrorOfficialActionTrace>,
    val mulliganActionCount: Int,
    val terminal: MonoBlueTerrorOfficialTerminal?,
    val telemetry: MonoBlueTerrorTelemetryIndex,
)

/**
 * Production gameplay driver. It accepts only an already-authorized initialized game, uses the
 * same qualified production AI profile as the accepted calibration, and submits exactly one engine
 * action per trace. It owns no artifact loader, assignment decoder, evidence path, seed source,
 * retry, replacement, or workflow surface.
 */
object PestControlTierOneMonoBlueTerrorProductionDriver {
    fun drive(
        registry: CardRegistry,
        officialGame: MonoBlueTerrorAuthorizedOfficialGame,
        maxActions: Int = 12_000,
        maxTurns: Int = 60,
        maxActionsPerTurn: Int = 500,
    ): MonoBlueTerrorOfficialRawGame {
        val environment = officialGame.environment
        val traces = mutableListOf<MonoBlueTerrorOfficialActionTrace>()
    ...
    fun encode(raw: MonoBlueTerrorOfficialRawGame): ByteArray =
    ...
    private fun submitExactlyOne(
        environment: com.wingedsheep.gym.GameEnvironment,
        action: GameAction,
        traces: MutableList<MonoBlueTerrorOfficialActionTrace>,
    ) {
        ...
        traces += MonoBlueTerrorOfficialActionTrace(
